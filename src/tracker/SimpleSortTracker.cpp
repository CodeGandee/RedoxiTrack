...
//
#include "RedoxiTrack/tracker/SimpleSortTracker.h"
...
{
void SimpleSortTracker::init(const TrackerParam &param)
{
    m_param = param.clone();
    auto p = dynamic_cast<SimpleSortTrackerParam *>(m_param.get());
...
const TrackerParam *SimpleSortTracker::get_tracker_param() const
{
    return TrackerBase::get_tracker_param();
}

void SimpleSortTracker::set_tracker_param(const TrackerParam &param)
{
    assert_throw(false, "not implemented");
}

void SimpleSortTracker::set_detection_comparision(
    const DetectionTraitsPtr &hander)
{
    m_detection_comparision = hander;
}

void SimpleSortTracker::reset_detection_comparision()
{
    m_detection_comparision = std::make_shared<DefaultDetectionTraits>(this);
}

void SimpleSortTracker::begin_track(const cv::Mat &img,
                                  ...
                                  int frame_number)
{
    ...
    for (auto det : detections) {
        ...
        TrackTargetPtr deepsort_target =
            create_target(det, frame_number, kalman_target);
...
void SimpleSortTracker::track(const cv::Mat &img,
                            const std::vector<DetectionPtr> &detections,
                            int frame_number)
{
    ...
    auto p_param = dynamic_cast<SimpleSortTrackerParam *>(m_param.get());
...
void SimpleSortTracker::track(const cv::Mat &img, int frame_number)
{
    assert_throw(m_frame_number != INIT_TRACKING_FRAME,
                 "m frame number is INIT_TRACKING_FRAME");
    assert_throw(m_frame_number <= frame_number,
    ...
    for (auto &p : m_id2target) {
        ...
        auto single_deepsort_target =
            dynamic_pointer_cast<SimpleSortTrackTarget>(temp_target);
        ...
        // No optical flow, so no need to update kalman, directly use kalman predicted bbox
...
void SimpleSortTracker::_update_features(SimpleSortTrackTargetPtr &target,
...
void SimpleSortTracker::_bbox2xyah(const BBOX &bbox, cv::Mat &output)
{
...
void SimpleSortTracker::delete_target(int path_id)
{
    m_id2target.erase(path_id);
}

void SimpleSortTracker::delete_all_targets()
{
}

void SimpleSortTracker::add_target(const TrackTargetPtr &target)
{
...
TrackTargetPtr SimpleSortTracker::create_target(const DetectionPtr &det,
                                              int frame_number)
{
    SimpleSortTrackTargetPtr output = std::make_shared<SimpleSortTrackTarget>();
...
TrackTargetPtr
    SimpleSortTracker::create_target(const DetectionPtr &det, int frame_number,
                                   const TrackTargetPtr &kalman_target)
{
    auto output = create_target(det, frame_number);
    auto p = dyncast_with_check<SimpleSortTrackTarget>(output.get());
    p->m_kalman_target = kalman_target;
    return output;
}

void SimpleSortTracker::add_event_handler(const TrackingEventHandlerPtr &handler)
{
    m_event_handlers.insert(handler);
}

void SimpleSortTracker::remove_event_handler(
    const TrackingEventHandlerPtr &handler)
{
    m_event_handlers.erase(handler);
}

void SimpleSortTracker::_match_maha_distance(
...
void SimpleSortTracker::_match_iou_distance(
    const std::vector<DetectionPtr> &sources,
    const std::vector<TrackTargetPtr> &targets,
    std::vector<std::pair<int, int>> &output_matched_pair,
    std::vector<int> &output_unmatched_source,
...
void SimpleSortTracker::_remove_targets(const int frame_number)
{
...
void SimpleSortTracker::_update_target(TrackTargetPtr &deepsort_target_ptr,
                                     const DetectionPtr &det,
                                     const int &frame_number)
{
    auto single_deepsort_target =
        dynamic_pointer_cast<SimpleSortTrackTarget>(deepsort_target_ptr);
...
void SimpleSortTracker::push_tracking_state()
{
...
void SimpleSortTracker::pop_tracking_state(bool apply)
{
...
int SimpleSortTracker::KalmanEventHandler::evt_target_association_after(
...
int SimpleSortTracker::KalmanEventHandler::evt_target_created_after(
    TrackerBase *sender, const TrackingEvent::TargetAssociation &evt_data)
{
...
int SimpleSortTracker::KalmanEventHandler::evt_target_motion_predict_after(
...
void SimpleSortTracker::set_feature_traits(const FeatureTraitsPtr &p)
{
    m_feature_traits = p;
}

FeatureTraitsPtr SimpleSortTracker::get_feature_traits()
{
    return m_feature_traits;
}

FeatureTraitsPtr
    SimpleSortTracker::DefaultDetectionTraits::get_feature_traits() const
{
    return m_tracker->get_feature_traits();
}

SimpleSortTracker::DefaultDetectionTraits::DefaultDetectionTraits(
    SimpleSortTracker *p)
    : m_tracker(p)
{
}
